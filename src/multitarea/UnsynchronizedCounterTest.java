...
/**
 * Variable compartida por muchos hilos, problema al modificarla simultaneamente
 * falla y no tiene el valor que deberia, solución poner synchronyzed en counter
 * cuando se llama a inc
 */
public class UnsynchronizedCounterTest {

	/**
	 * A class representing a counter with a method for incrementing the counter. No
	 * synchronization is used, so this counter is not "thread-safe".
	 */
	...
	/**
	 * The class that defines one of the threads. The thread simply increments
	 * counter numberOfIncrements times, in a for loop.
	 */
	...
	/**
	 * The main program runs in a loop until the user wants to exit. Each time
	 * through the loop, it runs one experiment. It gets the number of threads and
	 * the number of increments per thread from the user. It creates and starts the
	 * threads, and then waits for them all to finish. It prints the final value of
	 * the counter, as well as the expected value. The program ends when the user
	 * enters a number less than or equal to zero as the number of threads.
	 */
	public static void main(String[] args) {
		...
		while (true) {
			...
			/*
			 * Get number of threads and number of increments per thread from the user. Exit
