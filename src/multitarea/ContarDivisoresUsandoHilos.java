 ...
 * 
 * The program's work is divided among one to ten threads. The number of threads
 * is chosen by the user.
 */
public class ContarDivisoresUsandoHilos {
	...
	/**
	 * The largest number of divisors found so far. (Note: This is volatile since it
	 * is referenced in unsynchronized code in the countDivisorsWithThreads()
	 * method.)
	 */
	...
	/**
	 * This method is called by a thread when it has completed its computation, to
	 * report the largest number of divisors that it found in its assigned range of
	 * integers. The information is used to update the variables maxDivisorCount and
	 * intWithMaxDivisorCount.
	 * 
	 * @param maxCountFromThread   largest divisor count in the thread's assigned
	 *                             range
	 * @param intWithMaxFromThread the integer that had the maximum number of
	 *                             divisors
	 */
	...
	/**
	 * A thread belonging to this class counts the number of divisors for all the
	 * integers in an assigned range of integers. The range is specified in the
	 * constructor. The thread finds the integer in the range that has the largest
	 * number of divisors, and a number that has that many divisors. At the end of
	 * its computation, the thread reports its answer by calling the report()
	 * method.
	 */
